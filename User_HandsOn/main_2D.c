...
// !! TO BE MODIFIED !!
const pFuncIconDraw iconDrawFunctions[] = 
{
	DrawPrevious,
	 DrawStop,
	DrawPause,
...
{
       // GUI_SetColor(GUI_RED); //for advanced task, RED STOP button
        GUI_FillRoundedRect(x,y,size+x,size+y,10 ); //advanced task, rounded STO
...
void DrawNext(int x, int y, int size)
{
	GUI_POINT pPoint[3]; 
	pPoint[0].x = 0;
	pPoint[0].y = 0;
...
void DrawPrevious(int x, int y, int size)
{
	GUI_POINT pPoint[3]; 
	pPoint[0].x = (size/2);
	pPoint[0].y = 0;
...
void ReDrawIcon(GUI_COLOR color, int iconTouched)
{
	GUI_SetColor(color);
